...
function requestOptions(options, path, req, bodyContent, host, parseReqBody) {
  var parsedHost = (options.memoizeHost && parsedHost) ? parsedHost : parseHost(host, req, options);
...
function createRequestOptions(req, res, options, host) {
  var parsedHost = (options.memoizeHost && parsedHost) ? parsedHost : parseHost(host, req, options);
...
function bodyContent(req, res, options) {
  ...
  if (parseReqBody) {
    return maybeParseBody(req, options.limit || '1mb');
  }
}
